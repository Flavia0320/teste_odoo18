    ...
    def _createInvLines(self):
        ...
        if self.invoicing_type=='detailed':
            for task in self.task_ids:
                if task.task_transport_ids:
                    res += [(0, 0, {'name': _('Transport %s') % task.name, 'display_type':'line_section'})]
                    res += [
                    (0, 0, {
                        'name': " ".join([str(p.effective_kms), _("kms"), "-", p.transport_id.name]),
                        'price_unit': p.price_unit,
                        'quantity': p.effective_kms,
                    ...
                    }) for p in task.task_transport_ids]
        return res
